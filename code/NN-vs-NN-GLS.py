...
name = fun + 'phi%i' % phi + "sig%i" % (theta[0]) + "tau%i" % (int(100 * tau)) + 'mtd' + method + flexible_str + '_NN_inc-domain'
#name = 'test'
    ...
    for n in [100, 200, 500, 1000, 2000, 5000, 10000, 20000, 50000, 100000]:
        ...
        df_RMSE = pd.DataFrame(
            {#'BRISC': RMSE_BRISC, 'GAM': RMSE_GAM, 'GAM_krig': RMSE_GAM_krig, 'GAM_latlong': RMSE_GAM_latlong,
             ...
             'n': n_vec})
        #df_MISE.to_csv(".//simulation//NN//" + name + '_MISE.csv')
        df_RMSE.to_csv(".//simulation//NN//" + name + '_RMSE.csv')
        #df_GMSE.to_csv(".//simulation//NN//" + name + '_GMSE.csv')
