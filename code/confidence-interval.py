...
#name = 'test'
name = fun + 'phi%i' % phi + "sig%i" % (theta[0]) + "tau%i" % (int(100 * tau)) + 'mtd_' + resample + '_' + S
...
for rand in range(1, 101):
    ...
    mat[rand,:] = utils_NN.rmvn(1, np.zeros(n), cov, I_B, F_diag, Sparse)

CI_U, CI_L = np.quantile(mat, 0.975, axis=0), np.quantile(mat, 0.025, axis=0)
int_coverage(np.repeat(0, n), CI_U, CI_L)
'''
...
for rand in range(1, 101):
    ...
    _, _, model_NNGLS = utils_NN.train_decor_new(model_NNGLS, optimizer, data, 1000, theta_hat0, sparse = Sparse, sparseB=True,
    ...
    beta_hat_NNGLS, theta_hat_NNGLS = utils_NN.BRISC_estimation(residual_NNGLS_np, X[~data.test_mask,], coord[~data.test_mask,])
    ...
    MISE_NN = np.append(MISE_NN, RMSE(Est_MISE_NN, Y_MISE.reshape(-1)))
    MISE_NNGLS = np.append(MISE_NNGLS, RMSE(Est_MISE_NNGLS, Y_MISE.reshape(-1)))
    ####################################################################################################################
    if CI:
        ...
        for rep in range(n_rep):
            if resample == 'subsample':
                ...
                if Sparse:
                    ...
                    Y_sub_NNGLS = Y_hat_NNGLS + utils_NN.resample_fun_sparseB(residual_NNGLS, coord, nn, theta_hat_NNGLS)
                else:
            ...
            elif resample == 'oracle':
                torch.manual_seed(2023 + rep * rand)
                np.random.seed(2023 + rep * rand)
                id = range(n)
                X_sub = X
                #Y_sub = Y_hat + utils_NN.rmvn(1, np.zeros(n), cov, I_B_temp, F_diag_temp, sparse = Sparse, chol = False)
                Y_sub = Y_hat + np.std(residual_train) * np.random.randn(1, n)
                Y_sub_NNGLS = Y_hat_NNGLS + utils_NN.rmvn(1, np.zeros(n), cov_hat, I_B_temp, F_diag_temp, sparse=Sparse)
                coord_sub = coord
                I_B_sub = I_B
                F_diag_sub = F_diag
    ...
    CI_U_NNGLS = CI_U_NNGLS - np.mean(residual_NNGLS_np)
    CI_L_NNGLS = CI_L_NNGLS - np.mean(residual_NNGLS_np)
